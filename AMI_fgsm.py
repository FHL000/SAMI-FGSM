# coding=utf-8
"""Implementation of VMI-FGSM attack. 用来测试正太分布的，通过调stddev方差的参数"""
...
tf.flags.DEFINE_integer('number', 500, 'the number of images for variance tuning')  #求方差时x周围求平均的图片数

tf.flags.DEFINE_float('beta', 2.5, 'the bound for variance tuning.')
...
tf.flags.DEFINE_string('output_dir', './shiyan/xiaoro/zhengtai',
...
# 传入(x, one_hot, iter, max_iter, eps*FLAGS.beta, tf.zeros_like(new_grad))
def batch_grad(x, one_hot, i, max_iter, alpha, grad):  # 最后一个参数是1/N*global_grad中的global_grad
    #x_neighbor = x + tf.random.uniform(x.shape, minval=-alpha, maxval=alpha)  # alpha=0.012549  原论文均匀分布
    x_neighbor = x + tf.random.normal(x.shape,mean=0.0,stddev=alpha)   #取正太分布alpha=0.012549
    
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(  # logits_v3是shape=(10, 1001)的tensor
            x_neighbor, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
        cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)  # one_hot编码是上次迭代的X
    #with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
    #    logits_v4, end_points_v4 = inception_v4.inception_v4(
    #        x_neighbor, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
    #    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v4)        
        grad += tf.gradients(cross_entropy, x_neighbor)[0]  # 在x邻域内所取到的样本点的全部梯度之和  也就是globle_grad  也就是V(x)中的第一个
    i = tf.add(i, 1)
...
def graph(x, y, i, x_max, x_min, grad, variance):
    ...
    alpha = eps / num_iter  # 0.012549019607843137
    ...
    y = first_round * pred + (1 - first_round) * y  # y一开始是一个十行一列的全为0的  i大于1时的y就不是0了，是

    one_hot = tf.one_hot(y, num_classes)

    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)
    
    new_grad = tf.gradients(cross_entropy, x)[0]  # 计算参数(params)梯度
    iter = tf.constant(0)
    max_iter = tf.constant(FLAGS.number)
    _, _, _, _, _, global_grad = tf.while_loop(grad_finish, batch_grad, [x, one_hot, iter, max_iter, eps * FLAGS.beta,tf.zeros_like(new_grad)])  # 这里X除第一次，其余都是每次迭代攻击的样本
    # 返回和输入的命名都是一样的(x, one_hot, i, max_iter, alpha, grad)  里面的参数唯一只更新了grad
    
    current_grad = new_grad + variance  # 分子

    noise = momentum * grad + (current_grad) / tf.reduce_mean(tf.abs(current_grad), [1, 2, 3], keep_dims=True)  # 后面是分母

    variance = global_grad # / (1. * FLAGS.number) #- new_grad  # 这里是V(x)的计算公式
    #variance = 0.05*variance + global_grad / (1. * FLAGS.number) - new_grad
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_adv, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min,
                                                              tf.zeros(shape=batch_shape), tf.zeros(shape=batch_shape)])
        # Run computation
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            #kernel = tf.tile(kernel[:, :, tf.newaxis, tf.newaxis], [1, 1, 3, 1]) # tf.tile表示复制(拼贴)  tf.newaxis在原数组的维度上加一个维度             
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                idx = idx + 1
                print("start the i={} attack".format(idx))  # 每一次攻击是一个batch_size
                                  
                adv_images = sess.run(x_adv, feed_dict={x_input: images})
                save_images(adv_images, filenames, FLAGS.output_dir)
                diff = (adv_images + 1) / 2 * 255 - (images + 1) / 2 * 255    #将图片恢复到0~255之间
